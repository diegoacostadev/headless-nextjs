...
export const generateStaticParams = async () => {
  const data = await getCustomerPost();
  if (!data?.customerPostCollection.items.length) return null;
  const posts = data.customerPostCollection.items;
...
export default async function CustomerSingle({ params: { slug }}: { params: { slug: string } }) {
  const data = await getCustomerPost(slug);

  if (!data?.customerPostCollection.items.length) return;
            ...
            <div className="md:grow pb-12 md:pb-20">
              <div className="max-w-3xl">
                ...
                <article className="pb-12 mb-12 border-b [border-image:linear-gradient(to_right,transparent,theme(colors.slate.800),transparent)1]">
                  ...
                  <header>
                    <h1 className="h2 inline-flex bg-clip-text text-transparent bg-gradient-to-r from-slate-200/60 via-slate-200 to-slate-200/60 pb-4">{post.title}</h1>
                    ...
                    <div className="px-5 py-6">
                      <div className="mb-5">
                        <div className="flex items-center space-x-4">
                          <Image src={post.customer.logo.url} width={64} height={64} alt="Customer badge" />
                          <div className="text-lg font-semibold text-slate-100">{post.customer.name}</div>
                        </div>
                      </div>
